...
function LinearMixedModel(
    ...
    for (i, x) in enumerate(Xs)
        if isa(x, AbstractReMat{T})
            push!(reterms, x)
        elseif isa(x, DefaultReMat) # this can occur in weird situation where x is a ReMat{U}
...
function _pushALblock!(A, L, blk)
    push!(L, blk) # add blk to L (inplace)
    return push!(A, deepcopy(isa(blk, BlockedSparse) ? blk.cscmat : blk)) # add blk to A
end
...
function createAL(reterms::Vector{<:AbstractReMat{T}}, Xy::FeMat{T}) where {T}
    k = length(reterms) # number of random terms
    vlen = kchoose2(k + 1) # number of random effect terms (k) + 1 Xy term

    # build and stack all A and L blocks coming from the linear combinations of [Z X y]'[ Z X y]
    A = sizehint!(AbstractMatrix{T}[], vlen)
    L = sizehint!(AbstractMatrix{T}[], vlen)

    # all combinations random terms
    ...
    # [X y] * Z_i terms -> all combinations of FeTerms with ReMats
...
function setθ!(m::LinearMixedModel{T}, θ::AbstractVector) where {T}
    ...
    length(θ) == length(parmap) || throw(DimensionMismatch()) # check the number of parameters
    reind = 1
    λ = first(reterms).λ
...
function updateL!(m::LinearMixedModel{T}) where {T}
    A, L, reterms = m.A, m.L, m.reterms # A -> constant symbols for penalized least squares, L -> lower cholesky factor, reterms --> random Effect Terms
    k = length(reterms) # number of random effect terms
    copyto!(last(m.L), last(m.A))  # ensure the fixed-effects:response block is copied  -> in createAL the fixed effect block is added the last -> therefore we need to copy the last element
    
    #A'Z'WZA
    for j in eachindex(reterms) # pre- and post-multiply by Λ, add I to diagonal
        cj = reterms[j] # random effect term  -> lower covraiance factor to multiply with
        
        # Diagonal elements
        # get the blocks in A and L corresponding to that random effect
        diagind = kp1choose2(j) # gives the index of the lower cholesky factor
        
        # Update and inflate diagnonal elements to obtain  Λ'Z'WZΛ + I
        copyscaleinflate!(L[diagind], A[diagind], cj) 
        
        # Update non diagonal elements (this is why we iterate from j+1) of Λ'Z'WZΛ + I  (no +I required here because already done with previous step)
        # Postmultiply for non diagonal to obtain Z'WZΛ and X'WZΛ --> apply factor to rows of given column
        for i in (j + 1):(k + 1)     # postmultiply column by Λ --> goes through all blocks which are actually multiplied by this random effect
            bij = block(i, j)       # get the index of that block in A and and L (block index)
            rmulΛ!(copyto!(L[bij], A[bij]), cj) # postmultiply # Copy 
        end

        # Premultiply for non-diagnonal elements (oto obtain Λ'Z'WZΛ ) -> factor the non-diagonal row (apply factor to columns of given row)
